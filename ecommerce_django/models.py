...
class Item(TimeStampedModel, ActivatorModel, TitleSlugDescriptionModel, models.Model):
    ...
    image = models.ImageField(
        default="default_item_image.jpg", upload_to="items", null=True, blank=True
    )

    stock = models.IntegerField(default=1)
    variable_price = MoneyField(
        max_digits=14, decimal_places=2, default_currency="GBP", null=True, blank=True
    )
...
class CartItem(TimeStampedModel, ActivatorModel, models.Model):
    """
    ecommerce.CartItem
...
class Cart(TimeStampedModel, ActivatorModel, models.Model):
    """
    ecommerce.Cart
...
class Source(TimeStampedModel, ActivatorModel, models.Model):
    """
    ecommerce.Source
...
class Line(TimeStampedModel, ActivatorModel, models.Model):
...
class Invoice(TimeStampedModel, ActivatorModel, models.Model):
    """
    ecommerce.Invoice
...
class WalletManager(models.Manager):
    ...
    def for_source(self, source):
        ...
        """
        qs = self.get_query_set().filter(sources=source)
